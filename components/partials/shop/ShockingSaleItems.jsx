...
import ShockingProduct from "~/components/elements/products/ShockingProduct";
import ShockingProductWide from "~/components/elements/products/ShockingProductWide";
import ProductRepository from "~/repositories/ProductRepository";
import ModuleShockingSortBy from "~/components/partials/shop/modules/ModuleShockingSortBy";
import { useRouter } from "next/router";
import { generateTempArray } from "~/utilities/common-helpers";
import SkeletonProduct from "~/components/elements/skeletons/SkeletonProduct";

const ShockingSaleItems = ({ columns = 4, pageSize = 12 }) => {
  ...
  async function getProducts(params) {
    ...
    const responseData = await ProductRepository.getShockingProducts(payload);
  ...
  function handlePagination(page, pageSize) {
    ...
    if (
      ...
      Router.push("/shockingproducts" + newdd + "&page=" + page);
    } else {
      Router.push(`/shockingproducts?page=${page}`);
    }
  }
    ...
    if (productItems && productItems.length > 0) {
      if (listView) {
        ...
        productItemsView = productItems.map((item) => (
          <ShockingProductWide product={item} />
        ));
      }
    } else {
...
export default ShockingSaleItems;
