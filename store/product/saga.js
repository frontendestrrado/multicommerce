...
import {
  ...
  getAuctionProductByAuctionIdSuccess,
} from "./action";
polyfill();
...
function* getProductById({ id }) {
  try {
    ...
    yield put(getSingleProductsSuccess(product.data));
  } catch (err) {
    yield put(getProductsError(err));
  }
}
...
function* addProductToUserCart({}) {
  try {
    const result = yield call(ProductRepository.addProductToCart, payload);
...
function* getAuctionProductByAuctionIdSaga(payload) {
  try {
    const result = yield call(
      ProductRepository.getAuctionProductByAuctionId,
      payload
    );
    yield put(getAuctionProductByAuctionIdSuccess(result));
  } catch (err) {
    yield put(getProductsError(err));
  }
}
...
export default function* rootSaga() {
  ...
  yield all([
    takeEvery(actionTypes.GET_PRODUCTS_BY_PRICE_RANGE, getProductByPriceRange),
  ]);
  ...
  yield all([takeEvery(actionTypes.ADD_PRODUCT_TO_CART, addProductToUserCart)]);
  yield all([
    takeEvery(
      actionTypes.GET_AUCTION_PRODUCT_BY_AUCTION_ID,
      getAuctionProductByAuctionIdSaga
    ),
  ]);
}
